...
@section('breadcrumb_current', 'Жалобы')

@section('content')
<div class="box">
    <div class="box-header">
        <h3>Жалобы на сборки</h3>
        @if($reports->total() > 0)
    ...
    <div style="padding:48px; text-align:center; color:#6e7681;">
        ...
        <div style="font-size:12px; margin-top:4px;">Все жалобы рассмотрены.</div>
    </div>
    @else
    <table class="gh-table">
        ...
        <tr>
            <td>
                <a href="{{ route('builds.show', $report->build) }}" style="color:#58a6ff; text-decoration:none; font-weight:600; font-family:monospace; font-size:13px;">
                    {{ substr($report->build->uuid, 0, 8) }}
                </a>
                @if($report->build->packageVersion?->package)
                <div style="font-size:12px; color:#6e7681; margin-top:2px;">{{ $report->build->packageVersion->package->name }}</div>
                @endif
            </td>
            <td>
                <div style="font-size:13px; font-weight:600; color:#e6edf3;">{{ $report->build->user->name ?? '—' }}</div>
                <div style="font-size:11px; color:#6e7681; margin-top:2px;">
                    @php $bans = $report->build->user->ban_count ?? 0; @endphp
                    {{ $bans }} {{ $bans === 1 ? 'блокировка' : ($bans < 5 ? 'блокировки' : 'блокировок') }}
                    @if($report->build->user->isBanned())
            ...
            <td style="text-align:right; white-space:nowrap;">
                <form method="POST" action="{{ route('admin.reports.accept', $report) }}" style="display:inline;">
                    @csrf
                    <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Принять жалобу? Сборка будет заблокирована, издатель получит бан.')">
                        Принять
                    </button>
                </form>
                <form method="POST" action="{{ route('admin.reports.reject', $report) }}" style="display:inline; margin-left:4px;">
                    @csrf
